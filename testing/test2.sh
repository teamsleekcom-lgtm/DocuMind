    ...
    case "$build_type" in
        full)
            ...
            if [ "$enable_security" == "true" ]; then
                ...
                service_name="DocuMind-Security-Fat"
            else
                compose_file="${docker_compose_base}-fat${compose_suffix}"
                service_name="DocuMind-fat"
            fi
            ;;
        ultra-lite)
            ...
            if [ "$enable_security" == "true" ]; then
                ...
                service_name="DocuMind-backend-ultra-lite-security"
            else
                compose_file="${docker_compose_base}-ultra-lite${compose_suffix}"
                service_name="DocuMind-Ultra-Lite"
            fi
            ;;
    esac

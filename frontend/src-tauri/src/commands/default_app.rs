...
/// Check if DocuMind is the default PDF handler
#[tauri::command]
...
/// Attempt to set/prompt for DocuMind as default PDF handler
#[tauri::command]
...
fn check_default_windows() -> Result<bool, String> {
    ...
    unsafe {
        ...
        let result = (|| -> Result<bool, String> {
            ...
            // Check if it contains "DocuMind" (case-insensitive)
            // Note: This checks the ProgID registered by the installer
            let is_default = default_str.to_lowercase().contains("documind");
            Ok(is_default)
        })();
...
fn check_default_macos() -> Result<bool, String> {
    ...
    unsafe {
        ...
        let is_default = handler_str == "com.documind.app";
        Ok(is_default)
    }
}
...
fn set_default_macos() -> Result<String, String> {
    ...
    unsafe {
        ...
        let pdf_uti = CFString::new("com.adobe.pdf");
        let our_bundle_id = CFString::new("com.documind.app");
...
fn check_default_linux() -> Result<bool, String> {
    ...
    let is_default = handler.trim() == "documind.desktop";
    Ok(is_default)
}
...
fn set_default_linux() -> Result<String, String> {
    ...
    let result = Command::new("xdg-mime")
        .args(["default", "documind.desktop", "application/pdf"])
        .output()

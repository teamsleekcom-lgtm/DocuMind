...
fn get_log_directory() -> PathBuf {
    if cfg!(target_os = "macos") {
        // macOS: ~/Library/Logs/DocuMind
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
        PathBuf::from(home).join("Library").join("Logs").join("DocuMind")
    } else if cfg!(target_os = "windows") {
        // Windows: %APPDATA%\DocuMind\logs
        let appdata = std::env::var("APPDATA").unwrap_or_else(|_| std::env::temp_dir().to_string_lossy().to_string());
        PathBuf::from(appdata).join("DocuMind").join("logs")
    } else {
        // Linux: ~/.config/DocuMind/logs
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
        PathBuf::from(home).join(".config").join("DocuMind").join("logs")
    }
}
